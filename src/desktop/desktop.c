...
/*
   ...
   Copyright (c) Kostas Michalopoulos 2001. All Rights Reserved.

   Web site: http://software.xduffy.com
 ...
 * 02/06/2002: Various updates, merged in changed from Bad Desktop (orudge)
 */
...
l_bool enter_sleepmode();

static p_menuitem dicons_menuitems()
{
   return new_menu_item_ex("Open", NULL, NULL, MSG_ICON_OPEN,
                              true, NULL, NULL, NULL, NULL, NULL, 0,
          new_menu_item_ex("Sort Icons Horizontally", NULL, NULL, MSG_ICON_SORT_H,
                              true, NULL, NULL, NULL, NULL, NULL, 0,
          new_menu_item_ex("Sort Icons Vertically", NULL, NULL, MSG_ICON_SORT_V,
                              true, NULL, NULL, NULL, NULL, NULL, 0,
          new_menu_item_ex("Arrange Icons", NULL, NULL, MSG_ICON_SORT_G,
                              true, NULL, NULL, NULL, NULL, NULL, 0,
...
l_dword program_valid ( p_object o, l_dword msg )
{
   if (msg == MSG_QUIT)
   {
      if (msgbox(MW_QUESTION, MB_YES|MB_NO, TXT_AREYOUSUREYOUWANTTOQUITSEAL) != MSG_YES)
         return(!0);
...
l_bool enter_sleepmode()
{
   if(msgbox(MW_QUESTION, MB_YES|MB_NO, TXT_AREYOUSUREYOUWANTTOENTERSLEEPMODE) == MSG_YES)
      return true;
   return false;
}
...
//      bmp_path = get_key(set_format_text(NULL,"system/icons/%s/icon32", _strdup(p_drivers->drive)));
      ...
      HDM = *((long*)getini_fromfile(strdup("drives.ini"), set_format_text(NULL,"%s", _strdup(p_drivers->drive)), strdup("HDM")));
      DEBUG_printf("HDM: %i\n", (int)HDM);
      ...
      if(drive > -1 && HDM != 0)
      {
         ...
         DEBUG_printf("Loading of drivemeter for %i done.\n", drive);
         dicon->Meter_on = true;
      }
      else
         dicon->Meter = NULL;
...
lib_begin ( void ) {
  ...
  if ( ap_process == AP_INIT ) {
          ...
          image_menu = load_image(get_key("current/seal/images/sealmenu_image")); // OCR
...
SetInfoCopyright("Copyright (c) David Fredin 2002-2003, Kostas Michalopoulos 2000-2002, Owen Rudge 2002");
