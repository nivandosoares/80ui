...
/* Bad Desktop 0.55: Copyright (c) Kostas Michalopoulos 2001-2002 */

/* Revision History (as of 10/04/2002):
 *
 * 10/04/2002: Updated for SEAL 2.00.11 (orudge)
 * 11/04/2002: Continued updating (orudge)
 * 11/05/2002: Changed badsealmenu_image in registry to sealmenu_image (orudge)
 * 26/05/2002: Updated bmp dir path (orudge)
 */
...
#define  MSG_xCPL              100130
#define  MSG_LOADCPL           100131
...
/*************************************************/
/* FUNCTION: taskbar_windows                     */
/*                                               */
/* PURPOSE: To return a long variable saying how */
/*          many taskbar windows there are.      */
/*************************************************/
...
/**********************************************/
/* FUNCTION: taskbar_callback                 */
/*                                            */
/* PURPOSE: Callback routine for taskbar.     */
/*                                            */
/* PARAMETERS:                                */
/*                                            */
/* - o - The taskbar object                   */
/**********************************************/
...
/**********************************************/
/* FUNCTION: taskbar_translate_event          */
/*                                            */
/* PURPOSE: Event handler for taskbar.        */
/*                                            */
/* PARAMETERS:                                */
/*                                            */
/* - o     - The taskbar object               */
         ...
         do
         {
            if (z->tag & TAG_WINDOW)
            {
               ...
               if (mouse->where.x >= rr.a.x && mouse->where.x <= rr.b.x && mouse->where.y >= rr.a.y && mouse->where.y <= rr.b.y)
               {
                  ...
                  if (m)
                     menu = menuview_init(malloc(sizeof(t_menuview)), mr, m);
...
/**********************************************/
/* FUNCTION: taskbar_draw                     */
/*                                            */
/* PURPOSE: Taskbar drawing routine.          */
/*                                            */
/* PARAMETERS:                                */
/*                                            */
/* - o     - The taskbar view object          */
/**********************************************/
         ...
         do
         {
            if (z->tag & TAG_WINDOW)
            {
               ...
               if (z->state & OB_SF_FOCUSED)
               {
                  ...
                  if (z->tag & TAG_APPWIN)
                     draw_sprite(out, APPWIN(z)->icon16, rr.a.x+p.x+3, rr.a.y+p.y+2);
               ...
               else 
               {
                  ...
                  if (z->tag & TAG_APPWIN)
                     draw_sprite(out, APPWIN(z)->icon16, rr.a.x+p.x+2, rr.a.y+p.y+1);
...
/*                                            */
/* PURPOSE: Initialises the taskbar.          */
/*                                            */
/* PARAMETERS:                                */
/*                                            */
/* - o     - The taskbar object               */
/* - r     - The size and position of the bar */
/**********************************************/
...
/**********************************************/
/* FUNCTION: userbox_draw                     */
/*                                            */
/* PURPOSE: System tray drawing routine.      */
/*                                            */
/* PARAMETERS:                                */
/*                                            */
/* - o     - The system tray view object      */
/**********************************************/
...
/*                                             */
/* PURPOSE: Initialises the system tray.       */
/*                                             */
/* PARAMETERS:                                 */
/*                                             */
/* - o     - The system tray (userbox) object  */
/* - r     - The size and position of the tray */
/***********************************************/
...
p_userbox _userbox_init(p_userbox o, t_rect r)
{
   ...
   VIEW(o)->set_palette(VIEW(o), pal_taskbar); // too bored to make a new palette ;)
...
/*                                            */
/* PURPOSE: Refreshes the system tray.        */
/**********************************************/
...
/***********************************************/
/* FUNCTION: add_to_userbox                    */
/*                                             */
/* PURPOSE: Adds an object to the system tray. */
/*                                             */
/* PARAMETERS:                                 */
/*                                             */
/* - o     - The object to add                 */
...
/***********************************************/
/* FUNCTION: remove_from_userbox               */
/*                                             */
/* PURPOSE:  Removes an object from the system */
/*           tray.                             */
/*                                             */
/* PARAMETERS:                                 */
/*                                             */
...
/*****************************************************/
/* FUNCTION: new_filemenu_item_ex                    */
/*                                                   */
/* PURPOSE: Adds a new item to a file menu.          */
/*                                                   */
/* PARAMETERS:                                       */
/*                                                   */
/* - name        - The title of the menu item        */
/* - param       - Text for hotkey (I think)         */
/* - hotkey      - Hotkey to use                     */
/* - message     - Message used to identify item     */
/* - enable      - Is item enabled?                  */
/* - info_text   - Descriptive text for menu item    */
/* - flags       - Flags to set                      */
/* - font        - Font to use                       */
/* - icon        - Icon to be displayed next to item */
/* - font_symbol - Font for symbols                  */
/* - chr         - Character out of symbol font      */
/* - filename    - Filename                          */
/* - next        - Next menu item for 'chain'        */
/*****************************************************/
...
/*****************************************************/
/* FUNCTION: new_sub_filemenu_ex                     */
/*                                                   */
/* PURPOSE: Adds a new submenu to a file menu.       */
/*                                                   */
/* PARAMETERS:                                       */
/*                                                   */
/* - name        - The title of the menu item        */
...
/***********************************************/
/* FUNCTION: menu_load_from_dir                */
/*                                             */
/* PURPOSE:  Loads a menu of files from a      */
/*           directory.                        */
/*                                             */
/* PARAMETERS:                                 */
/*                                             */
/* - path     - The path to search             */
/* - filename - The files to search for        */
/*                                             */
/* RETURNS:  A pointer to the generated menu   */
/***********************************************/
...
/***********************************************/
/* FUNCTION: get_quit_items                    */
/*                                             */
/* PURPOSE:  Returns the 'Shut Down' menu item */
/*           'section'.                        */
/*                                             */
/* PARAMETERS:                                 */
/*                                             */
/* - next    - The next menu item in the tree  */
/*                                             */
/* RETURNS:  A pointer to the generated menu   */
/***********************************************/
...
/***********************************************/
/* FUNCTION: get_programs_items                */
/*                                             */
/* PURPOSE:  Returns the 'Programs' menu item  */
/*           'section'.                        */
/*                                             */
/* RETURNS:  A pointer to the generated menu   */
/***********************************************/
...
{
   if (!program_items)
      program_items = menu_load_from_dir(DIR_PROGRAMS, "*.*");
...
/*                                             */
/* PURPOSE:  Returns the 'Settings' menu item  */
/*           'section'.                        */
/*                                             */
/* RETURNS:  A pointer to the generated menu   */
/***********************************************/
...
{
   p_menuitem items = 0;
   p_menuitem menuitem = 0;

//   if (!settings_items)
//   {
      settings_items = new_menu_item(TXT_CONTROL_PANEL, 0, 0, MSG_xCPL, NULL,
         NULL); /* new_menu_line(NULL));

// THIS CODE IS INCOMPLETE!
      if (cp_sections)
      {
         p_item panel = cp_sections->first(cp_sections);  // Get first control panel section
         p_item first_panel = panel;

         if (panel)
         {
            do
            {
               p_cp_section cps = CP_SECTION(panel->rec);

               if (cp_loaders && cp_loaders->last)       // Get first control panel item
               {
                  p_item cpitem = cp_loaders->first(cp_loaders);
                  p_item first_item = cpitem;

                  do
                  {
                     p_cp_loader cploader = CP_LOADER(cpitem->rec);

                     if (cploader->sec == cps)  // TODO: Fix this!
                     {
                        menuitem = new_menu_item_ex(cploader->caption, NULL, NULL, MSG_LOADCPL, true, NULL, 0, NULL, cploader->icon16, NULL, NULL, NULL);

                        if (!items)
                           items = menuitem;
                        else 
                           add_menu_item_to_menuitem(items, menuitem);
                     }
                  
                     cpitem = cpitem->next;
                  } while (cpitem != first_item);

                  settings_items = add_menu_item_to_menuitem(settings_items, new_sub_menu(cps->caption, new_menu(items), NULL)); 
               }

               panel = panel->next;
            } while (panel != first_panel);
         }
      }*/

//      settings_items = menu_load_from_dir(DIR_SETTINGS, "*.*");
      return(settings_items);
//   }
}

/***********************************************/
/* FUNCTION: get_document_items                */
/*                                             */
/* PURPOSE:  Returns the 'Documents' menu item */
/*           'section'.                        */
/*                                             */
/* RETURNS:  A pointer to the generated menu   */
/***********************************************/
...
{
   if (!docs_items)
      docs_items = menu_load_from_dir(DIR_DOCS, "*.*");
...
/*                                                */
/* PURPOSE:  Returns the 'Help' menu item section */
/*                                                */
/* RETURNS:  A pointer to the generated menu      */
/**************************************************/
...
{
   if (!help_items)
      help_items = menu_load_from_dir(DIR_HELP, "*.*");

   return(help_items);
}

/***********************************************/
/* FUNCTION: get_startment_items               */
/*                                             */
/* PURPOSE:  Returns the Start menu items.     */
/*                                             */
/* RETURNS:  A pointer to the generated menu   */
/***********************************************/
...
static p_menuitem get_startmenu_items(void)
{
  p_menuitem p = new_sub_menu("Programs", new_menu(get_programs_items()),
                 ...
                 new_menu_line(
                 new_menu_item("About Bad Desktop...", 0, 0, MSG_xABOUT, NULL,
                 new_menu_line(
     ...
     switch ( event->message ) {
        ...
        case MSG_xAPPLICATION : {
            ...
            if ( menu ) {
              ...
              switch (msg) {
                case MSG_xABOUT : {
                  seal2_app_about("Bad Desktop", load_image("/system/bmp/icons/desktop.ico,32"));
...
/*void icons_on_file(p_bad_filebox o, l_text file)
{
  run_file(file);
};

void icons_on_folder(p_bad_filebox o, l_text folder)
{
  DLXLoad(strdup("./splorer.exe"), folder);
};*/

void iconbox_on_dclick(p_bad_iconbox o, l_int item)
{
   switch (item)
   {
      case 0:         // OCR
...
lib_begin ( void ) {
  ...
  if ( ap_process == AP_INIT ) {
          ...
          image_menu = load_image(get_key("current/seal/images/sealmenu_image"));
...
//          desktopline_minimalized = list_init(sf_malloc(sizeof(t_list)), ((void (*)(void*))&free_menu_item), DAT_TEXT);
          ...
          r = rect_assign(0, r.b.y+1, desktop->bounds.b.x, desktop->bounds.b.y-26);
          icons = bad_iconbox_init(malloc(sizeof(t_bad_iconbox)), r, BIB_DESKTOP_PLACEMENT);

          VIEW(icons)->brush.color = registry_color("desktop_face"); // OCR: was desktop_background
          VIEW(icons)->brush.color2 = registry_color("desktop_face");
          VIEW(icons)->brush.background = load_image(get_key("current/desktop/wallpaper")); // OCR: 10/04/2002
          if (VIEW(icons)->brush.background) icons->flags |= BIB_BACKGROUND_IMAGE;

          icons->on_dclick = &iconbox_on_dclick;

          icons->add(icons, ICO_GET(2), "My Computer");
          icons->add(icons, load_image("/system/bmp/icons/dir.ico,32"), "Documents"); // OCR
//          icons->add(icons, ICO_GET(3), "Hard Disk");
//          icons->add(icons, ICO_GET(1), "Bad Seal");
//          icons->add(icons, ICO_GET(1), "Sources");

          OBJECT(desktop)->insert(OBJECT(desktop), OBJECT(icons));
...
SetInfoAppName("Bad Desktop");
SetInfoDescription("Bad Desktop for SEAL 2.0");
SetInfoCopyright("Copyright (c) Kostas Michalopoulos 2000-2002, Owen Rudge 2002");
