...
void DLXAddInAppList ( hdlx_t id, CDLX* o, char* filename ) { // *
  ...
  if ( o->info.appname > 0 )
    ...
    ap->appname      = strdup("Unknown");

  if ( o->info.description > 0 )
    ap->desciption   = strdup(o->data+o->info.description);
  else
    ap->desciption  = strdup("Unknown");

  if ( o->info.copyright > 0 )
    ap->copyright    = strdup(o->data+o->info.copyright);
  else
    ap->copyright    = strdup("Unknown");

  if ( o->info.manufacturer > 0 )
    ap->manufacturer = strdup(o->data+o->info.manufacturer);
  else
    ap->manufacturer = strdup("Unknown");

  memcpy(&ap->ver_major, &o->info.ver_major, sizeof(unsigned char));
  memcpy(&ap->ver_minor, &o->info.ver_minor, sizeof(unsigned char));
  memcpy(&ap->ver_revision, &o->info.ver_revision, sizeof(unsigned char));
  memcpy(&ap->ver_build, &o->info.ver_build, sizeof(unsigned char));
...
//  memcpy(&ap->version,&o->info.version,sizeof(long));
  ...
  DEBUG_printf("XDL :: Start running %s (%s) XDL %d%d - %d free memory bytes\n",ap->filename,ap->appname,ap->id,info_free_memory());
};
////////////////////////////////////////////////////////////////////////////////
char DelInAppList ( hdlx_t id ) { // *
  ...
  if ( listapp && listapp->last ) {
    ...
    do {
       ...
       if ( o->id == id ) {
         DEBUG_printf("XDL :: Unload XDL %d%d named %s (%s) - %d free memory bytes\n",id,o->appname,o->filename,info_free_memory());
