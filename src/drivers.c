 ...
 * 25/05/2002: Updating bmp path (orudge)
 */
...
void step_init ( ) {
  if ( lowgrfx ) {
    ...
    l_int gstep = 0;
    step_ico = load_image("bmp/bitmaps/step.bmp");
    gstep_ico = load_image("bmp/bitmaps/gstep.bmp");
    if ( gstep_ico ) {
...
t_drivers *drivers_init ( t_drivers *o ) {
  ...
  if ( !lowgrfx ) ld = load_image("bmp/bitmaps/load.bmp");
