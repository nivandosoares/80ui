 ...
 *
 * 29/04/2002: Tidied up appwin_setup and made widget skinning possible (orudge)
 * 02/05/2002: Fixed bug with minimize button using max down image (orudge)
 * 03/06/2002: Started merging in AbSEAL window skinning code (orudge)
 * 18/01/2003: Got new skinning almost done (xduffy)
 */
...
void bar_button_draw_state(p_button object, l_int press)
{
   ...
   if (out)
   {
      if (SKINFOR(object))
      {
         ...
         if(object->message == MSG_CLOSE)
            {
            if (press)
               skinitem = strdup("CloseButtonDown");
            else
            {
               skinitem = strdup("CloseButton");
            }
         }
         else if(object->message == MSG_MAXIMIZE)
            {
            if (press)
               skinitem = strdup("MaximizeButtonDown");
            else
            {
               skinitem = strdup("MaximizeButton");
            }
         }
         else if(object->message == MSG_MINIMIZE)
            {
            if (press)
               skinitem = strdup("MinimizeButtonDown");
            else
            {
               skinitem = strdup("MinimizeButton");
            }
         }
         else if(object->message == MSG_SYSMENU)
            {
            if (press)
               skinitem = strdup("MinimizeButtonDown");
            else
...
void systemsmenu_draw_state(p_button o, l_int press)
{
   ...
   if (out)
   {
   	...
   	else
      {
         ...
         if (ib->icon) /* center icon if exist */
         {
            ...
            if (o->caption)
               textout_draw_rect(out,VIEW(o)->font,o->caption,-1,p.x+(idwn?1:0),p.y+dy+icon->h,p.x+r.b.x+(idwn?1:0),p.y+dy+icon->h+get_text_height(VIEW(o)->font),TX_ALIGN_CENTER,color_3d_text,TX_NOCOLOR,1);
...
void appwin_setup (p_object o)
{
   ...
   b = button_init(_malloc(sizeof(t_button)), r, "", MSG_CLOSE, BF_PUSH+BF_NORMAL);
   b->draw_state = &bar_button_draw_state;
   if (b) /* make close button */
   {
      OBJECT(b)->set_options(OBJECT(b), VW_OF_IGNORELIM, true); /* ignore size limits of window */
      OBJECT(b)->set_options(OBJECT(b), OB_OF_SELECTABLE, false); /* can't be selected */
//         VIEW(b)->font = get_font_in_size("Symbols", 10, 10); /* font is symbol TTF font in size 10,10 */
      VIEW(b)->align |= TX_ALIGN_RIGHT+TX_ALIGN_FIXEDX; /* move button, when window is growing, but still fixed size */
   }
   ...
   if (WINDOW(o)->flags & WF_MAXIMIZE)
   {
      ...
      r = rect_move(r, -18, 0);
      b = button_init(_malloc(sizeof(t_button)), r, "", MSG_MAXIMIZE, BF_PUSH+BF_NORMAL);
      b->draw_state = &bar_button_draw_state;
...
//            VIEW(b)->font = get_font_in_size("Symbols", 10, 10);
   ...
   if(WINDOW(o)->flags & WF_MINIMIZE)
   {
         /* minimize button */
      r = rect_move(r, -18, 0);
      b = button_init(_malloc(sizeof(t_button)), r, "", MSG_MINIMIZE, BF_PUSH+BF_NORMAL);
      b->draw_state = &bar_button_draw_state;
      ...
      if(b)
      {
         ...
         OBJECT(b)->set_options(OBJECT(b), VW_OF_IGNORELIM, true);
            /* can't be selected */
         OBJECT(b)->set_options(OBJECT(b), OB_OF_SELECTABLE, false);
            /* font is symbol TTF font in size 10,10 */
//            VIEW(b)->font = get_font_in_size("Symbols", 10, 10);
            /* move button, when window is growing, but still fixed size */
         VIEW(b)->align |= TX_ALIGN_RIGHT+TX_ALIGN_FIXEDX;
      }
   ...
   if (WINDOW(o)->flags & WF_SYSMENU)
   {
      ...
      BUTTON(b)->draw_state = &systemsmenu_draw_state;
  ...
  if ( out ) {
    ...
    if (!SKINFOR(o)) { /* no skin */
      ...
      if (OBJECT(o)->state & OB_SF_SELECTED)
        DRAW_SKIN_RECT(out, SKINFOR(o), strdup("ActiveWindow"), rect_move(r, p.x, p.y));
      else
        DRAW_SKIN_RECT(out, SKINFOR(o), strdup("InactiveWindow"), rect_move(r, p.x, p.y));
    }
...
void  appwin_draw_title ( p_window o ) {
  ...
  if (!SKINFOR(o)) {
    r = rect_assign(2, 2, rect_sizex(VIEW(o)->bounds)-2, FONT_GETHEIGHT(VIEW(o)->font)+3);
  } else {
    p_skin_item si = find_skin_item(SKINFOR(o), "ActiveWindow");
    if (!si)
      r = rect_assign(2, 2, rect_sizex(VIEW(o)->bounds)-2, FONT_GETHEIGHT(VIEW(o)->font)+3);
    else
    {
//      r = rect_assign(si->left, si->top, rect_sizex(VIEW(o)->bounds)-si->right, FONT_GETHEIGHT(VIEW(o)->font)+si->top+si->bottom);
      r = rect_assign(0, 0, rect_sizex(VIEW(o)->bounds), FONT_GETHEIGHT(VIEW(o)->font)+si->bottom);
    }
  };

  safe = r;

  out = VIEW(o)->begin_paint(VIEW(o), &p, safe);
  ...
  if ( out ) {
    ...
    if (!SKINFOR(o)) {
      l_color bcolor1 = VIEW(o)->get_color(VIEW(o), 7);
      l_color bcolor2 = VIEW(o)->get_color(VIEW(o), 8);
      l_int   right = p.x+r.b.x-18;

      if ( OBJECT(o)->state & OB_SF_SELECTED ) {

        fcolor = VIEW(o)->get_color(VIEW(o), 6);
        bcolor1 = VIEW(o)->get_color(VIEW(o), 4);
        bcolor2 = VIEW(o)->get_color(VIEW(o), 5);

      };

//      rectfill(out, r.a.x+p.x, r.a.y+p.y, r.b.x+p.x, r.b.y+p.y, VIEW(o)->get_color(VIEW(o), 0));

      fade_rect(out, r.a.x+p.x+1, r.a.y+p.y+1, r.b.x+p.x-1, r.b.y+p.y-1, bcolor1, bcolor2, FR_VER);

      button(out, r.a.x+p.x, r.a.y+p.y, r.b.x+p.x, r.b.y+p.y, VIEW(o)->get_color(VIEW(o), 2), VIEW(o)->get_color(VIEW(o), 1));

      textout_draw_rect(out, VIEW(o)->font, caption, -1, r.a.x+p.x + 25, r.a.y+p.y+
      3,
                             r.b.x+p.x+50, r.b.y+p.y-1, TX_ALIGN_CENTER, fcolor, TX_NOCOLOR, 0);
    } else {
      l_text            skinitem = strdup("InactiveTitleBar");
      p_skin_item       si = NULL;
      l_int             right, diff;
      long margin_left;
      ...
      if (OBJECT(o)->state & OB_SF_SELECTED) {
        free(skinitem);
        skinitem = strdup("ActiveTitleBar");
      };

      si = find_skin_item(SKINFOR(o), skinitem);


      margin_left = *((long*)getini_fromfile(get_key("CURRENT/SEAL/SKIN/FILE"), strdup("ActiveTitleBar"), strdup("margin-left")));
      r.a.x = r.a.x + (l_int)margin_left;
      --r.b.x;
      DRAW_SKIN_RECT_EX(out, SKINFOR(o), skinitem, rect_move(r, p.x, p.y), VIEW(o)->font, caption);
    }
