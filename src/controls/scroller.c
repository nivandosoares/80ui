...
/* Revision History (as of 18/01/2003):
 *
 * 18/01/2003: Added skin support for scroller (xduffy)
 */
...
void   vscrollbar_draw_bar(p_vscrollbar object)
{
   p_view  vo = VIEW(object);
   t_rect  r = vo->size_limits(vo);
   t_rect  s = r;
   t_point p;

   l_rect  pos  = object->get_bar_pos(object);
   l_rect  size = object->get_bar_size(object);
   ...
   if(out)
   {
      ...
      if(SKINFOR(object))
      {
         ...
         skinitem = strdup("vScrollBar");

         DRAW_SKIN_RECT(out, SKINFOR(object), skinitem, rect_move(r, p.x, p.y));
         free(skinitem);
      }
      else
      {
...
void hscrollbar_draw_bar(p_hscrollbar object)
{
   p_view  vo = VIEW(object);
   t_rect  r = vo->size_limits(vo);
   t_point p;
