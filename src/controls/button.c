 ...
 * 16/04/2002: icobuttons now skinned (Sean O'Brien)
 * 12/05/2002: Added AbSEAL skinning support (orudge)
 * 26/05/2002: Finished converting to AbSEAL skin system (orudge)
 */
...
void button_draw_state (p_button object, l_int press)
{
   ...
   if (out)
   {
      if (SKINFOR(object))
      {
         ...
         if (press)
            skinitem = strdup("PressedButton");
         else
         {
            if ((object->flags & BF_UNDERSEL) && (object->flags & BF_OVER))
               skinitem = strdup("HoverButton");
            else
               skinitem = strdup("Button");
         }
...
void button_draw (p_view object)
{
   BUTTON(object)->draw_state(BUTTON(object), false);
}
...
void icobutton_draw_state (p_button o, l_int press)
{
   ...
   if (out)
   {
      if (SKINFOR(o))
      {
         ...
         skin = find_skin_item(SKINFOR(o), skinitem);

         DRAW_SKIN_RECT_EX(out, SKINFOR(o), skinitem, rect_move(r, p.x, p.y), NULL, NULL);
         free(skinitem);
         ...
         if (ib->icon)  /* center icon if exist */
         {
            ...
            if (o->caption)
               textout_draw_rect(out, VIEW(o)->font, o->caption, -1, p.x + (idwn ? 1 : 0), p.y + dy + icon->h, p.x + r.b.x + (idwn ? 1 : 0), p.y + dy + icon->h + get_text_height(VIEW(o)->font), TX_ALIGN_CENTER, skin->textcolor, TX_NOCOLOR, 1); // skin->textcolor was "color_3d_text"
         }
      }
	else
      {
