 ...
 *
 * 03/05/2002: Updated XDL Task Manager to support new XDL version system (orudge)
 * 26/05/2002: Changed bmp/... to bmp/icons/... (orudge)
 */
...
l_int   SealApiVersion = 200012; // 2.00.012
l_text  SealApiTxtVersion = "2.00.12";
...
// At the moment, no different to DEBUG_printf
l_int DEBUG_printf_ex (l_int type, l_text text, ...)
{
   l_int nc = 0;
   va_list arg;
...
void seal2_app_about(l_text appname, BITMAP *icon)
{
   ...
   if (icon == NULL)
      app_about_bmp = load_image("/system/bmp/icons/app.ico,32");
   else
      app_about_bmp = icon;
...
void ListLoadedApp ( void ) {
  ...
  if ( listapp && listapp->last ) {
    ...
    if  ( f )
    do {
       p_app object = ((p_app)(x->rec));

       DEBUG_printf ("  XDL no %d %d : %s (%s) by %s\n",object->id ,object->appname,object->filename,object->manufacturer);
...
l_int  txt_seal_error ( l_int errtype, l_text string, ... )
{
...
p_list get_loaded_xdl ( void ) {
  ...
  if ( p ) {
    ...
    do {
       ...
       p->insert(p, new_listbox_item(set_format_text(NULL,"%s (%s - %d.%02d.%d.%d)",o->appname,o->filename,o->ver_major, o->ver_minor, o->ver_revision, o->ver_build), NULL, 0, NULL));
...
#define MSG_END_OF_SEAL 9998 // Neither here ;-) (xduffy 25/01/2003)
...
void xdl_man_trans_ev(p_object o, p_event event)
{
   if(event->type & EV_MESSAGE)
   {
      ...
      else if(msg == MSG_END_OF_SEAL)
      {
//         program.valid = 0;
         set_event(event, EV_MESSAGE, MSG_QUIT, OBJECT(desktop));
...
void  fast_trans_ev ( p_object o, p_event event ) {
  ...
  if ( event->type == EV_KEYBOARD && OBJECT(keyb)->state & KB_SF_KEYDOWN && KEYCTRL(KEY_F12) ) {
    ...
    lst = listbox_init( _malloc(sizeof(t_listbox)),rect_assign(5,20,r.b.x-20,r.b.y-75),
                        get_loaded_xdl(),1,LF_SELFLIST);
    XDL_LISTBOX = lst;
    OBJECT(win)->insert(OBJECT(win),OBJECT(lst));

    txt = stattext_init( _malloc(sizeof(t_stattext)),rect_assign(5,r.b.y-70,r.b.x-5,r.b.y-55),
    ...
    b = button_init(_malloc(sizeof(t_button)),
                            rect_assign(5,r.b.y-50,(r.b.x/2)-5,r.b.y-30),
                            "End Task",
                            MSG_END_OF_TASK,
                            BF_DEFAULT);
    ...
    b = button_init(_malloc(sizeof(t_button)),
                            rect_assign((r.b.x/2)+5,r.b.y-50,r.b.x-5,r.b.y-30),
                            "(x) Close",
                            MSG_CLOSE,
                            BF_DEFAULT);
    OBJECT(win)->insert(OBJECT(win), OBJECT(b));
    ...
    b = button_init(_malloc(sizeof(t_button)),
                            rect_assign(5,r.b.y-25,r.b.x-5,r.b.y-5),
                            "Quit Seal",
                            MSG_END_OF_SEAL,
                            BF_DEFAULT);
    OBJECT(win)->insert(OBJECT(win), OBJECT(b));
