...
/* Revision History (as of 03/05/2002):
 *
 * 03/05/2002: Made version customisable from object file, instead of being hard-coded to
 *             time. Split into four parts - major, minor, revision and build. (orudge)
 *
 * 10/05/2002: Built DLXINSD into DLXGEN and added support in resource script for datafile
 *             to be appended automatically (orudge)
 *
 */
...
int append_datafile(FILE *output, char *infile);
...
int main(int argc, char **argv)
{
  ...
  if (argc < 3)
  {
    printf("XDL Generator - Version 1.2 - Supports XDL and CXDL
  dlxgen [-C]output.xdl [-Rinput.srs] input.o [input2.o ... -lgcc -lc ...icons.dat]
    [-C]        : Activate Compression
    output.xdl  : Output file
    -Rinput.srs : SEAL Resource Script
    [ ... ]     : See LD help\n");
    return 1;
  }

  if (argv[2][0] == '-' && (argv[2][1] == 'R' || argv[2][1] == 'r'))
  {
     if (argc < 4)
     {
        printf("XDL Generator - Version 1.2 - Supports XDL and CXDL
  ...
  info.need_seal_version = (SEAL_VER_MAJOR*6) + (SEAL_VER_MINOR*5) + SEAL_VER_REVISION; //200011;

  if (resfile)
  {    
     info.ver_major = getininum_fromfile(resfile, "Version", "Major");
     info.ver_minor = getininum_fromfile(resfile, "Version", "Minor");
     info.ver_revision = getininum_fromfile(resfile, "Version", "Revision");
     info.ver_build = getininum_fromfile(resfile, "Version", "Build");

     update = getininum_fromfile(resfile, "Version", "AutoIncrementBuild");

     if (update == 1)
     {
        info.ver_build++;
        tmpver = info.ver_build;

        setini_tofile(resfile, "Version", "Build", &tmpver, INI_DECANUM);
     }

 //    printf("Ver: %d.%d.%d.%d\n", info.ver_major, info.ver_minor, info.ver_revision, info.ver_build);

     datafile_appended = getininum_fromfile(resfile, "Datafile", "Append");

     if (datafile_appended == 1)
        datafile_filename = getini_fromfile(resfile, "Datafile", "Filename");
  }
  else
  {
     info.ver_major = 1; //time(NULL);
     info.ver_minor = 0;
     info.ver_revision = 0;
     info.ver_build = 0;
...
//      printf("bss_start 0x%x\n",bss_start);
  ...
  if (datafile_appended == 1)
  {
     filesize += append_datafile(output_f, datafile_filename);
     printf("  + datafile:   : %d KB\n", filesize / 1024);
  }
...
int append_datafile(FILE *output, char *infile)
{
   ...
   if (!data)
   {
      printf("\nError: Not enough memory (%ld bytes) for datafile.\n", ilen);
      fclose(input);

      return(0);
   }

   fread(data, ilen, 1, input);  // Read entire datafile into memory
   fclose(input);

   fwrite(data, ilen, 1, output);  // Write datafile to XDL file
