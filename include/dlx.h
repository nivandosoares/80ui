...
/*
   ...
   07/04/2002 - Fixed C++ issue with lib_end (orudge)
   03/05/2002 - Made version in info structure seperate bytes instead of long, and added
                SetInfoVersion macro (orudge)
*/
...
#include "engine.h"
  ...
  typedef struct t_app {        // *
    ...
    unsigned char ver_revision;
    unsigned char ver_build;
//    long     version;  // 03/05/2002 - Changed from long to bytes (orudge)
    char*     desciption;
    char*     copyright;
    char*     manufacturer;
...
#define lib_end                              } DLX_EF;      // OCR - 07/04/2002
...
#define SetInfoVersion(major, minor, revision, build)   \
   unsigned char InfoVersionMajor = major; \
   unsigned char InfoVersionMinor = minor; \
   unsigned char InfoVersionRevision = revision; \
   unsigned char InfoVersionBuild = build; \
