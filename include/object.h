...
#define TXT_PLEASESTOREYOURDATAANDEXITSOMEAPPLICATION INI_TEXT("Please store your data and exit some applications")
#endif
...
/* clipboard, which must be freed in all copy function */
...
/* object functions */
extern t_object *(*obj_init) (t_object *p_object1);
t_object *_obj_init(t_object *p_object1);

l_bool   obj_done(t_object *p_object1);

t_object *obj_find_match_view(t_object *p_view, l_dword state, l_dword options, l_bool search_forward);
...
t_object *obj_next_view(t_object *p_view);
...
t_object *obj_prev_to_first(t_object *p_object1);
...
void     obj_set_options(t_object *p_object1, l_dword options, l_bool set);
l_bool   obj_is_state(t_object *p_object1, l_dword state);
l_bool   obj_is_options(t_object *p_object1, l_dword options);

l_int    obj_put_into_stillprocess(p_object o, p_object s);
l_int    obj_clear_from_stillprocess(p_object o, p_object s);
/* I recommend that we remove these functions for SEAL 3.0 */
...
t_object *obj_at(p_object o, l_long index);
...
void     init_stillprocess(p_object p_object1, l_int milliseconds);
#define  done_stillprocess(o) init_stillprocess(o, -1)

void     set_event(t_event *event, l_dword type, l_dword message, t_object *p_event_object);
void     set_event_info(t_event *event, l_dword type, l_dword message, t_object *p_event_object, void *info);
